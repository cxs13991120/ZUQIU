...
1. 确认生产仓库是 `cxs13991120/ZUQIU`，生产分支是 `main`。
...
4. 创建 fine-grained personal access token。`Repository access` 选择 **Only select repositories**，并且只选择 `cxs13991120/ZUQIU`；`Repository permissions` 只设置：
   - Metadata: Read-only
...
- `GITHUB_OWNER`：填写仓库所有者 `cxs13991120`。
- `GITHUB_REPO`：填写仓库名 `ZUQIU`。
- `GITHUB_TOKEN`：填写上一步创建的 fine-grained token，只能在自己的 Apps Script 项目中填写。
- `REPORT_STATUS_URL`：仓库路径 `web/report-status.json` 位于 Pages artifact 根目录中，因此填写公开地址 `https://cxs13991120.github.io/ZUQIU/report-status.json`，不能在域名后再加 `/web/`。
- `REPORT_IMAGE_URL`：仓库路径 `web/daily-report.png` 同样位于 artifact 根目录中，因此填写 `https://cxs13991120.github.io/ZUQIU/daily-report.png`。
- `REVALIDATION_INDEX_URL`：仓库路径 `web/revalidation-index.json` 会发布到 Pages artifact 根目录，因此填写 `https://cxs13991120.github.io/ZUQIU/revalidation-index.json`，不要加 `/web/`。索引中的 `web/revalidation/DATE/status.json` 和 revision PNG 会按同一 artifact 根目录解析。
- `REPORT_SITE_URL`：`web/index.html` 是 artifact 首页，因此填写 `https://cxs13991120.github.io/ZUQIU/`。
- `RECIPIENT_EMAIL`：填写接收日报的邮箱地址，填入你的 QQ 邮箱：`594241761@qq.com`。
...
7. 在 GitHub 打开 **Actions**，选中 `Daily Sporttery Forecast`，点击 **Run workflow**，分支选 `main`，通过 `workflow_dispatch` 输入当天北京时间日期（`YYYY-MM-DD`）。按当天缺少的阶段继续手动运行 `Draw Alert Refresh` 和 `Afternoon Sporttery Settlement`。完成后检查仓库路径 `web/report-status.json`，并打开公开地址 `https://cxs13991120.github.io/ZUQIU/report-status.json`，确认 `report_date` 是当天日期、阶段状态为 `true`、`build_id` 非空；从 `https://cxs13991120.github.io/ZUQIU/daily-report.png` 下载同一构建的 PNG，在 PowerShell 运行 `Get-FileHash .\daily-report.png -Algorithm SHA256`，确认结果与状态文件中的 `image_sha256`（PNG 的 SHA-256）一致。报告首页是 `https://cxs13991120.github.io/ZUQIU/`。
...
先把 `TEST_MODE` 改为 `true`。创建新的 fine-grained token，仍然只授权 `cxs13991120/ZUQIU`，权限仍然只有 Metadata: Read-only 和 Actions: Read and write；替换 `GITHUB_TOKEN` 后手动运行 `runAutomation` 并检查日志。确认成功后设置 `TEST_MODE=false`，再在 GitHub 撤销旧令牌。不要把新旧令牌写进日志或工单。
