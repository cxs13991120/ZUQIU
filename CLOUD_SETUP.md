...
| `.github/workflows/draw-alert-refresh.yml` | 13:30 | 刷新市场数据与平局预警；独立刷新步骤失败时继续执行，并用仓库中最新可用文件重建报告。 |
...
可选市场来源出错但采集器仍正常完成时，`collect_market_heat.py` 会记录错误、保留仍通过验证的来源；后续预警只按本次可用来源和质量门槛生成，绝不补造缺失数据。若 13:30 的导入、预测、市场采集或预警生成步骤真正以非零状态失败，工作流会继续；该步骤未成功替换的 12:15 已提交产物仍留在仓库，随后网站和图片按仓库中最新可用文件重建。这里是按文件保留与重建，不会把整份报告自动回滚到某个旧版本。

这套机制减少旧报告被覆盖或过早发送的风险，但不能保证外部数据源始终成功。结果来源延迟时，结算会保留未结算状态并在后续任务重试；不应把缺失来源当作有效市场证据。
